...
def execute_concurrent_test(operation: str, filename: str, num_clients: int, server_workers: int) -> Dict:
    """Run concurrent test with thread pool executor"""
    start_time = time.time()
    results = []
...
def format_result_for_display(result: Dict) -> str:
    ...
    lines = []
    lines.append(f"\nThread Results for {result['operation']} {result['filename']}:")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description='Thread-based stress test')
    parser.add_argument('--server-workers', type=int, choices=[1, 5, 50], 
                       help='Number of server worker threads')
